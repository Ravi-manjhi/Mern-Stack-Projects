...
export const getTours = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const tours = await feature?.query.populate({
      path: "guides",
      model: userModel,
...
export const getToursWithin = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const radius =
      unit === "mi" ? parseInt(distance) / 3963.2 : parseInt(distance) / 6378.2;

    if (!latitude || !longitude)
      return next(new AppError("Please provide like this lat,lng", 400));
